...
class DaoTestRound implements DaoRoundInterface
{
    ...
    const GROUPROUNDS_PATH = "DataAccessObjects\\Round\\Mock\\GROUPROUNDS.xml";
    ...
    public function getAllRounds(int $championshipId): array
    {
        ...
        $result = $database->getFilteredObjects(self::ROUND, array(
    ...
    public function getRoundsByGroup(int $groupId): array
    {
        $rounds = array();
        $database = new XMLInterface(self::PATH);
        $groupsDatabase = new XMLInterface(self::GROUPROUNDS_PATH);
        $result = $groupsDatabase->getFilteredObjects("GROUPROUNDS", array(
            "IDGROUP" => $groupId
        ));
        $roundsIds = [];
        foreach ($result as &$item) {
            $roundsIds[] = $item["IDROUND"];
        }

        $roundsDb = $database->getFilteredObjects(self::ROUND, array(
            self::ID => $roundsIds
        ));

        foreach ($roundsDb as &$item) {
            $rounds[$item[self::ID]] = $this->convertToRound($item);
        }
