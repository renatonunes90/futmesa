...
(
  ...
  CONSTRAINT UNQ_CHAMPIONSHIP UNIQUE (NAME)
);
...
(
  ...
  IDWINNER Integer,
  CONSTRAINT UNQ_RESULTGAME UNIQUE (IDGAME)
);
...
ALTER TABLE CHAMPIONSHIPHISTORIC ADD CONSTRAINT "FK_CH_ROUND"
...
ALTER TABLE GAME ADD CONSTRAINT FK_GAME_ROUND
  FOREIGN KEY (IDROUND) REFERENCES ROUND (ID) ON UPDATE CASCADE ON DELETE CASCADE;
ALTER TABLE GAMERESULT ADD CONSTRAINT FK_GAMERESULT_GAME
  FOREIGN KEY (IDGAME) REFERENCES GAME (ID) ON UPDATE CASCADE ON DELETE CASCADE;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON GAMERESULT TO  SYSDBA WITH GRANT OPTION;
