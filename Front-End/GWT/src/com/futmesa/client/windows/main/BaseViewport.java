...
/**
 * App main viewport. Contains header menu and main container.
 */
public final class BaseViewport implements IsWidget {

	private static final ModulesViewportUiBinder uiBinder = GWT.create(ModulesViewportUiBinder.class);

	interface ModulesViewportUiBinder extends UiBinder<FluidContainer, BaseViewport> {
	}
	...
	/**
	 * Menu de campeonatos. Instância dinamicamente seus subitens conforme os
	 * campeonatos cadastrados no sistema.
	 */
	private MenuBar championshipMenu;

	/**
	 * Menu de jogadores. Instância dinamicamente seus subitens conforme os
	...
	private void createConfigMenu() {
		/*
		 * configMenuBar.setAutoOpen( true ); configMenuBar.setAnimationEnabled( true );
		 * 
		 * MenuBar mainMenu = new MenuBar( true ); mainMenu.setAnimationEnabled( true );
		 * 
		 * // item raiz vazio somente para abrir os submenus MenuItem mainItem = new
		 * MenuItem( "   ", mainMenu ); mainItem.setPixelSize( 24, 24 );
		 * configMenuBar.addItem( mainItem );
		 * 
		 * Command menuCommand = new Command() { public void execute() { URLFilter
		 * filter = new URLFilter( Modules.CONFIG_MODULE,
		 * MainModulePanel.CHAMPIONSHIP_PANEL ); Window.Location.assign(
		 * filter.toURLString() ); } }; mainMenu.addItem( new MenuItem(
		 * constants.manageChampionshipMenu(), menuCommand ) );
		 * 
		 * menuCommand = new Command() { public void execute() { URLFilter filter = new
		 * URLFilter( Modules.CONFIG_MODULE, MainModulePanel.PLAYER_PANEL );
		 * Window.Location.assign( filter.toURLString() ); } }; mainMenu.addItem( new
		 * MenuItem( constants.managePlayersMenu(), menuCommand ) );
		 */
	}
	 ...
	 * 
	 * @param panel Nova interface que deve ser exibida.
	 */
